...
export default function FirebaseApps() {
	...
	async function createNew(values) {
		...
		setAddLoading(true);
		try {
			await addDoc(collection(db, 'app_configs'), values);
		...
		<div style={{ padding: 16 }}>
				...
				{loading ? (
					...
					<div style={{ textAlign: 'center', padding: 40, color: '#888' }}>暂无配置</div>
						...
						<Table
							...
							columns={[
							{ title: 'app_id', dataIndex: 'app_id', key: 'app_id', width: 180 },
							{ title: 'name', dataIndex: 'name', key: 'name', width: 200 },
							{ title: 'type', dataIndex: 'app_type', key: 'app_type', width: 120 },
							{
								title: 'download_url', dataIndex: 'download_url', key: 'download_url', render: (text) => (
									<div>
										<Typography.Text copyable={{ text: text || '' }}>{text || ''}</Typography.Text>
										{text ? <div><a href={text} target="_blank" rel="noreferrer">打开</a></div> : null}
									</div>
								),
							},
							{
								title: 'market_url', dataIndex: 'market_url', key: 'market_url', render: (text) => (
									<div>
										<Typography.Text copyable={{ text: text || '' }}>{text || ''}</Typography.Text>
									</div>
								),
			...
			>
				<Form form={addForm} layout="vertical" initialValues={emptyItem}>
					<Form.Item name="app_id" label="app_id" rules={[{ required: true, message: '请填写 app_id' }]}>
						<Input />
					</Form.Item>
					<Form.Item name="name" label="name">
						<Input />
					</Form.Item>
					<Form.Item name="app_type" label="app_type">
						<Select>
					...
					<Form.Item name="download_url" label="download_url">
						<Input />
					</Form.Item>
					<Form.Item name="market_url" label="market_url">
						<Input />
					</Form.Item>
					<Form.Item name="onelink" label="onelink">
						<Input />
					</Form.Item>
					<Form.Item name="launch_url" label="launch_url">
						<Input />
					</Form.Item>
